...
- **Training levers this round:** KD recipe retained (`distill_ens_s19` + denser `distill_ens4_s23`); longer val-B-oriented tight (`tight_s53` from tight_s43 teacher); **val-A/B gate** before ens4 ship; mixup/distill/SWA/finetune retained.
...
| Split | Prior ship (6a3a426) | **Now** top-1 | **Now** top-5 |
|-------|----------------------|---------------|---------------|
| Val (WLASL100) | 49.7% | **51.2%** | **74.9%** |
| Test (WLASL100) | 46.5% | **47.3%** | **72.9%** |

243-class full head. **Plain top-1 47.3%** vs 46.5% @6a3a426 (**+0.8 pp**). Kept KD + gated ens; added longer `tight_s53` + denser ship-ens4 distill; **val-A/B gated** weighted ens4 (s7 + tight_s43 + distill_ens_s19 + tight_s53 → one Core ML) clears gates (val-A 54.4% / val-B 47.9%) with test 47.3%. Still short of ≥50% / conversation.
...
|------|---------|--------|-------|-------|
| Full (ships as primary) | 243 | WLASL100 holdout | **47.3%** | **72.9%** |
...
# then val-A/B gate WeightedLogitEnsemble(s7, tight_s43, distill_ens_s19, tight_s53) → export one Core ML only if both slices lift + test > ship
